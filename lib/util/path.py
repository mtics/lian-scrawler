...
def create_district_path(site, district):
    site_path = create_site_path(site)
    city_path = site_path + "/" + district
    if not os.path.exists(city_path):
        os.makedirs(city_path)
    return city_path


def create_date_path(site, district, date):
    district_path = create_district_path(site, district)
    date_path = district_path + "/" + date
